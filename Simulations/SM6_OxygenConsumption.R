...
save(results.nobio, file='SM6_nobioturbation.RData')

#====== BIOTURBATION PARAMETERS: WEAK BIOMIXING =======#
#Biodiffusion grid
Db.0   <- 1.0    # biodiffusion coefficient Db at SWI [cm2 yr-1]
L.mix  <- 1 + 9*(1-exp((-Db.0/3))) # Mixed depth layer
Db.inf <- 0.0      # asymptotic Db at depth cm2 yr-1
Db.x.att <- 2      # attenuation depth [cm]
Db.grid <- setup.prop.1D(func=p.sig,grid=grid,y.0=Db.0,y.inf=Db.inf,x.L=L.mix,x.att=Db.x.att)
...
results.weakbiomix <- model(t=0,state=out$y,parameters=parameters,full.output=TRUE)
rate.summary.weakbiomix <- results.weakbiomix$rate.matrix
reac.summary.weakbiomix <- results.weakbiomix$reac.matrix
results.weakbiomix$y <- out$y
save(results.weakbiomix, file='SM6_weakbiomixing.RData')
...
results.weakbioirr <- model(t=0,state=out$y,parameters=parameters,full.output=TRUE)
rate.summary.weakbioirr <- results.weakbioirr$rate.matrix
reac.summary.weakbioirr <- results.weakbioirr$reac.matrix
results.weakbioirr$y <- out$y
save(results.weakbioirr, file='SM6_weakbioirrigation.RData')


#====== BIOTURBATION PARAMETERS: WEAK BIOTURBATION =======#
#Biodiffusion grid
Db.0   <- 1.0    # biodiffusion coefficient Db at SWI [cm2 yr-1]
L.mix  <- 1 + 9*(1-exp((-Db.0/3))) # Mixed depth layer
Db.inf <- 0.0      # asymptotic Db at depth cm2 yr-1
...
#Bioirrigation grid
L.irr     <- 0  # irrigation depth [cm]
irr.0     <- 36.5 # irrigation rate at SWI [yr-1] #weak=36.5, strong=365
irr.inf   <- 0  # deep irrigation rate [yr-1]
irr.x.att <- 3.0  # irrigation attenuation coef [cm]
irr.grid <- setup.prop.1D(func=p.exp,grid=grid,y.0=irr.0,y.inf=irr.inf,x.L=L.irr,x.att=irr.x.att)
...
results.weak <- model(t=0,state=out$y,parameters=parameters,full.output=TRUE)
rate.summary.weak <- results.weak$rate.matrix
reac.summary.weak <- results.weak$reac.matrix
results.weak$y <- out$y
save(results.weak, file='SM6_weakbioturbation.RData')

#====== BIOTURBATION PARAMETERS: STRONG BIOMIXING =======#
#Biodiffusion grid
...
results.strongbiomix <- model(t=0,state=out$y,parameters=parameters,full.output=TRUE)
rate.summary.strongbiomix <- results.strongbiomix$rate.matrix
reac.summary.strongbiomix <- results.strongbiomix$reac.matrix
results.strongbiomix$y <- out$y
save(results.strongbiomix, file='SM6_strongbiomixing.RData')

#====== BIOTURBATION PARAMETERS: STRONG BIOIRRIGATION =======#
#Biodiffusion grid
Db.0   <- 0.0     # biodiffusion coefficient Db at SWI [cm2 yr-1]
L.mix  <- 1 + 9*(1-exp((-Db.0/3))) # Mixed depth layer
Db.inf <- 0.0      # asymptotic Db at depth cm2 yr-1
...
results.strongbioirr <- model(t=0,state=out$y,parameters=parameters,full.output=TRUE)
rate.summary.strongbioirr <- results.strongbioirr$rate.matrix
reac.summary.strongbioirr <- results.strongbioirr$reac.matrix
results.strongbioirr$y <- out$y
save(results.strongbioirr, file='SM6_strongbioirrigation.RData')

#====== BIOTURBATION PARAMETERS: STRONG BIOTURBTAION =======#
#Biodiffusion grid
...
results.strong <- model(t=0,state=out$y,parameters=parameters,full.output=TRUE)
rate.summary.strong<- results.strong$rate.matrix
reac.summary.strong <- results.strong$reac.matrix
results.strong$y <- out$y
save(results.strong, file='SM6_strongbioturbation.RData')
