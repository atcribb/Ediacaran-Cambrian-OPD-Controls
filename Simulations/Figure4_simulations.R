...
CH2O.fast.parms <- c(rep(F.CH2O.f.seq[1],3), rep(F.CH2O.f.seq[2],3), rep(F.CH2O.f.seq[3],3))
CH2O.slow.parms <- c(rep(F.CH2O.s.seq[1],3), rep(F.CH2O.s.seq[2],3), rep(F.CH2O.s.seq[3],3))
CH2O.tot.parms  <- CH2O.fast.parms + CH2O.slow.parms

#Set up bioturbation sequences
Db.0.parms <- rep(c(0, 1, 10),3)      #biodiffusion  coefficients
irr.0.parms <- rep(c(0, 36.5, 365),3) #bioirrigation coefficients
bioturbation_labels <- rep(c('None', 'Weak', 'Strong'),3)

#set up dataframe to save results
results_df <- as.data.frame(matrix(data=NA, nrow=length(CH2O.tot.parms), ncol=5))
colnames(results_df) <- c('F.CH2O', 'Bioturbation', 'Db', 'irr', 'OPD')
...
for(i in 1:length(CH2O.tot.parms)){
  ...
  results_df$OPD[i]           <- O2.penetrationdepth
  
}

#save data:
results.low <- results_df
save(results.low, file="Figure4_lowend.RData")
write_xlsx(results.low, 'Output_Figure4_lowend.xlsx')
...
#save data:
results.high <- results_df
save(results.high, file="Figure4_highend.RData")
write_xlsx(results.high, 'Output_Figure4_highend.xlsx')
